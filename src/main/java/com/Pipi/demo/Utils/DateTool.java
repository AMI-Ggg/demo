...
public final class DateTool {
	...
	private static final String FORMAT_STR_TIME4 = "MMdd";

	public static String getDateToMD(Date aDate) {
		SimpleDateFormat df = new SimpleDateFormat(FORMAT_STR_TIME4);
	...
	public static String getDateToYMD2(Date aDate) {
	...
	public static String strToHms(String strDate) {
		if (strDate != null && strDate.length() == 14) {
			String strFormat = strDate.substring(0, 4) + "-" + strDate.substring(4, 6) + "-" + strDate.substring(6, 8) + " " + strDate.substring(8, 10) + ":"
	...
	public static String hmsToStr(String strDate) {
		if (strDate != null && strDate.length() == 19) {
			String strFormat = strDate.substring(0, 4) + strDate.substring(5, 7) + strDate.substring(8, 10) + strDate.substring(11, 13) + strDate.substring(14, 16)
	...
	public static Date stringToDate(String strDate) {
		...
		try {
			...
			log.error("Could not convert '" + strDate + "' to a date, throwing exception, " + pe.getMessage());
		}
		return aDate;
	}
	...
	public static Date stringToDateYMDHMS(String dateString) {
		return stringToDate(dateString, FORMAT_STR_DATETIME, LENIENTDATE);
	}
	...
	public static String getDateToGMTStr(Date date) {
		SimpleDateFormat sdf = new SimpleDateFormat(FORMAT_STR_US, Locale.US);
	...
	public static int getHourBetween(Date sourceDate, Date targetDate) {
		if (sourceDate.before(targetDate)) {
			...
			return Integer.parseInt(String.valueOf((sourceMill - targetMill) / (60 * 60 * 1000)));
		}
	}

	public static int getDayBetween(Date sourceDate, Date targetDate) {
		if (sourceDate == null || targetDate == null || sourceDate.before(targetDate)) {
			return 0;
		} else {
			Calendar cal = Calendar.getInstance();
			cal.setTime(sourceDate);
			...
			cal.setTime(targetDate);
			long targetMill = cal.getTimeInMillis();
			return Integer.parseInt(String.valueOf((sourceMill - targetMill) / (24 * 60 * 60 * 1000)));
		}
	}
